...
if __name__ == '__main__':
    ...
    # 初始化服务车辆
    init_SVs = vehicle.get_SVs(args_parser().InitSVNum)
    ...
    for t in range(args_parser().T):
        #  1. 机会车辆到达, 进行微服务部署
        OVNum = np.random.poisson(OVNum_lambda)
        ...
        for RV in init_RVs:
            RV.requestID, RV.timestamp = random.randint(1, len(Request_MSIDs_List)-1), t
